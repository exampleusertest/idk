...
makefolder("tc2-cheat")

if not isfile("tc2-cheat/Interlude.mp3") then
    print("[ tc2-cheat ] 'Interlude.mp3' not found, downloading..")

    local Time = tick()
    local Interlude = request({Url = "https://github.com/geoduude/bibulus-dependencies/raw/main/Interlude.mp3"}).Body

    if Interlude then
        print("[ tc2-cheat ] 'Interlude.mp3' Downloaded in "..string.format("%.2f", tick() - Time).."s.")
        writefile("tc2-cheat/Interlude.mp3", Interlude)
    else
        warn("[ tc2-cheat ] 'Interlude.mp3' failed to load.")
    end
end

if not isfile("tc2-cheat/More Gun Reversed.mp3") then
    print("[ tc2-cheat ] 'More Gun Reversed.mp3' not found, downloading..")

    local Time = tick()
    local Interlude = request({Url = "https://github.com/geoduude/bibulus-dependencies/raw/main/More%20Gun%20Reversed.mp3"}).Body

    if Interlude then
        print("[ tc2-cheat ] 'More Gun Reversed.mp3' Downloaded in "..string.format("%.2f", tick() - Time).."s.")
        writefile("tc2-cheat/More Gun Reversed.mp3", Interlude)
    else
        warn("[ tc2-cheat ] 'More Gun Reversed.mp3' failed to load.")
    end
end

local InterludeID = getcustomasset("tc2-cheat/Interlude.mp3")
local MoreGunReversedID = getcustomasset("tc2-cheat/More Gun Reversed.mp3")
...
-- List of specific user IDs to track
local SpecialPlayers = {
    [90807940] = "game mod", -- Replace with actual user IDs and desired tags
    [956533729] = "potential ban perms",
    [3639439701] = "smarty pants cheat detector",
    [20622249] = "she will report you for the slightest anything",
    [260036781] = "game mod",
    [492538800] = "game mod",
    [9624991] = "game mod",
    [1095419] = "game mod",
    [71305063] = "game mod",
    [430335792] = "game mod",
    [87056092] = "mechanic VA, say hi and be nice.",
}

for _, Player in game.Players:GetPlayers() do
    task.spawn(function()
        -- Announce based on GroupRank
        repeat task.wait() until Player:GetAttribute("GroupRank")
        for i, v in Tags do
            if i == Player:GetAttribute("GroupRank") then
                Library:Notify("[ tc2-cheat ] ⚠ '".. Player.DisplayName .. " (@".. Player.Name ..")' is ranked '".. v .."' in the Dorcus Digital Group.", 15)
        ...
        -- Announce based on User ID
        if SpecialPlayers[Player.UserId] then
            Library:Notify("[ tc2-cheat ] 🌟 '".. Player.DisplayName .. " (@".. Player.Name ..")' is a '".. SpecialPlayers[Player.UserId] .."'.", 15)
            local Sound = Instance.new("Sound")
            Sound.Parent = game:GetService("SoundService")
            Sound.SoundId = "rbxassetid://8784885431"
            Sound.Volume = 5
...
MiscDebug:AddButton('tp to empty server', function()
    game.ReplicatedStorage.Functions.TeleportTo:InvokeServer("sisyphyustest.tc")
end)
            ...
            elseif self.Name == "BeanBoozled" or self.Name == "WEGA" or (string.find(string.lower(self.Name), "ban") and not self.Name == "UseBanner") then
                ...
                if Toggles.MD_notifyblacklistedremotes.Value then
                    for i,v in Options.MD_blacklistedremotes.Value do
                        if i == self.Name then
                            warn("[ tc2-cheat ] Blacklisted remote blocked: ".. self.Name)
                            game.StarterGui:SetCore("SendNotification", { Title = "[ tc2-cheat ]", Text = "Blacklisted remote blocked: ".. self.Name, Duration = 5 })
...
Options.MG_dmgmultiplier:OnChanged(function()
    local oldReturnDamageMod = require(LocalPlayer.PlayerGui.GUI.Client.Functions.Weapons).returndamagemod
    require(LocalPlayer.PlayerGui.GUI.Client.Functions.Weapons).returndamagemod = function()
...
ThemeManager:SetFolder('tc2-cheat')
ThemeManager:ApplyToTab(Tabs.Config)
SaveManager:SetLibrary(Library)
SaveManager:SetFolder('tc2-cheat/TypicalColors2')
